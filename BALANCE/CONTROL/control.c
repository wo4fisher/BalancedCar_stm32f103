...
{   
	...
	int balance;
	//角度环
	AngleBias=ZHONGZHI-Angle;       //===求出平衡的角度中值 和机械相关
#if CascadePid
...
{
	if(moto1>0)			PWMRB=0,			PWMRF=1100+moto1;  //右轮
...
{ 
	...
	{
		/****由于6050安装方向不一样，所以修改了读数********/
		Gyro_Y=(I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_XOUT_H)<<8)+I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_XOUT_L);    //读取Y轴陀螺仪
		Gyro_Z=(I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_ZOUT_H)<<8)+I2C_ReadOneByte(devAddr,MPU6050_RA_GYRO_ZOUT_L);    //读取Z轴陀螺仪
		Accel_X=(I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_YOUT_H)<<8)+I2C_ReadOneByte(devAddr,MPU6050_RA_ACCEL_YOUT_L); //读取X轴加速度计
